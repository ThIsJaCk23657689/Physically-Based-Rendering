...
public:
    ...
    template<class U>
    RefCountPtr(const RefCountPtr<U>& other, typename std::enable_if<std::is_convertible<U*, T*>::value, void*>::type * = 0) noexcept :
        ptr_(other.ptr_) {
        InternalAddRef();
    }
    ...
    template<class U>
    RefCountPtr(RefCountPtr<U>&& other, typename std::enable_if<std::is_convertible<U*, T*>::value, void*>::type * = 0) noexcept :
        ptr_(other.ptr_) {
        other.ptr_ = nullptr;
    }
    // -------------------------------
    ...
    void Attach(InterfaceType* other) noexcept {
        if (ptr_ != nullptr) {
            ...
            // Otherwise, re-attaching will cause the pointer to be released and may cause a crash on a subsequent dereference.
